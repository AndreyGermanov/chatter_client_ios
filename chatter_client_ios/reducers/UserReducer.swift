...
func userReducer(action:Action,state:UserState) -> UserState {
    ...
    switch action {
    case let action as changeUserIsLoginAction:
        newState.isLogin = action.isLogin
    case let action as changeUserLoginAction:
        newState.login = action.login
    case let action as changeUserEmailAction:
        newState.email = action.email
    case let action as changeUserFirstNameAction:
        newState.first_name = action.firstName
    case let action as changeUserLastNameAction:
        newState.last_name = action.lastName
    case let action as changeUserGenderAction:
        newState.gender = action.gender
    case let action as changeUserBirthDateAction:
        newState.birthDate = action.birthDate
    case let action as changeUserProfileImageAction:
        newState.profileImage = action.profileImage
    case let action as changeUserDefaultRoomAction:
        newState.default_room = action.default_room
    case let action as changeUserUserIdAction:
        newState.user_id = action.user_id
    case let action as changeUserSessionIdAction:
