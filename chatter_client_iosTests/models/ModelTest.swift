...
class ModelTest: XCTestCase {
    ...
    func testCompare() {
        let message = appStore.state.chat.messages[1].copy()
        XCTAssertTrue(ChatMessage.compare(model1: message, model2: appStore.state.chat.messages[1]), "Should return true if compare message and it copy")
        message.to_user?.room?.name = "CHANGED"
        XCTAssertFalse(ChatMessage.compare(model1: message, model2: appStore.state.chat.messages[1]), "Should return false if changed one of subfields")
    ...
    func testCompareLists() {
        ...
        var original_messages = appStore.state.chat.messages
        let message1 = original_messages[0]
        ...
        XCTAssertFalse(ChatMessage.compare(models1: messages, models2: appStore.state.chat.messages), "Should return false if number of items different")
        messages = original_messages.copy()
        messages[1].attachment = nil
        XCTAssertFalse(ChatMessage.compare(models1: messages, models2: appStore.state.chat.messages), "Should return false if property is nil")
        XCTAssertFalse(ChatMessage.compare(models1: messages, models2: nil), "Should return false if compare to nil")
        XCTAssertTrue(ChatMessage.compare(models1: nil, models2: nil), "Should return true if compare nils")
    }
}
